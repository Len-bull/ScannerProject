...
public final class DecodeThread extends Thread {
  ...
  DecodeThread(CaptureView captureView,
    ...
    if (decodeFormats == null || decodeFormats.isEmpty()) {
            ...
            switch (captureView.getDecodeType()){

                case CaptureView.DecodeType.ALL:
                    decodeFormats.addAll(DecodeFormatManager.ONE_D_FORMATS);
                    decodeFormats.addAll(DecodeFormatManager.QR_CODE_FORMATS);
                    decodeFormats.addAll(DecodeFormatManager.DATA_MATRIX_FORMATS);
                    break;

                case CaptureView.DecodeType.QRCODE:
                    decodeFormats.addAll(DecodeFormatManager.QR_CODE_FORMATS);
                    break;

                case CaptureView.DecodeType.ISBN:
                    decodeFormats.addAll(DecodeFormatManager.ISBN);
                    break;
